...
describe("useMetrics Hook", () => {
  ...
  it("should track component mount and unmount", () => {
    const { unmount } = renderHook(() => useMetrics({ componentName: "TestComponent" }));
  ...
  it("should provide trackAction function", () => {
    const { result } = renderHook(() => useMetrics({ componentName: "TestComponent" }));
  ...
  it("should provide trackAsyncAction function", async () => {
    ...
    let operationResult;
    await act(async () => {
      operationResult = await result.current.trackAsyncAction("api_call", asyncOperation(), {
        endpoint: "/api/test",
      });
    });
...
describe("useApiMetrics Hook", () => {
  ...
  it("should track successful API calls", async () => {
    ...
    let apiResult;
    await act(async () => {
      apiResult = await result.current.trackApiCall("GET", "/api/patients", mockApiCall);
    });
